defmodule Margarine.Python.SdxlServer do
  ...
  ## Example
      ...
      # Encode prompt
      {:ok, embeds} = SdxlServer.encode_prompt("a red panda eating bamboo")

      # Generate latents
      {:ok, latents} = SdxlServer.generate_latents(1024, 1024, seed: 42)

      # Run UNet step
      {:ok, noise} = SdxlServer.unet_forward(
        latents,
        500,
      ...
      # Decode to image
      {:ok, image} = SdxlServer.vae_decode(latents)
  """
  ...
  ## Options
    ...
    * `:model_type` - Required. Either `:sdxl_base` or `:sdxl_turbo`
    * `:model_id` - Required. HuggingFace model ID
    * `:device` - Device to run on ("mps", "cuda", "cpu"). Default: "mps"
    * `:torch_dtype` - PyTorch dtype ("float16", "float32", "bfloat16"). Default: "float16"
  ...
  def initialize_model(opts) do
    ...
    torch_dtype = Keyword.get(opts, :torch_dtype, "float16")
  ...
  ## Options
    ...
    * `:negative_prompt` - Negative prompt text. Default: ""
    * `:guidance_scale` - CFG guidance scale. Default: 7.5

  ## Returns

    * `{:ok, %{prompt_embeds: tensor, negative_embeds: tensor, pooled_embeds: tensor, negative_pooled_embeds: tensor}}` - Success
  ...
  def encode_prompt(prompt, opts) when is_binary(prompt) do
    case validate_prompt(prompt) do
      :ok ->
        # Note: negative_prompt will be used when SDXL is fully implemented
        _negative = Keyword.get(opts, :negative_prompt, "")
        guidance = Keyword.get(opts, :guidance_scale, 7.5)

        case validate_encode_options(guidance) do
          :ok ->
  ...
  @doc """
  Run one forward pass through SDXL UNet.

  ## Arguments

    * `latents` - Current latent tensor (Nx.Tensor)
    * `timestep` - Current timestep (integer, scheduler-specific)
  ...
  ## Options
    ...
    * `:height` - Target height for time_ids. Default: 1024
    * `:width` - Target width for time_ids. Default: 1024
  ...
  """
  @spec unet_forward(Nx.Tensor.t(), number(), Nx.Tensor.t(), Nx.Tensor.t(), keyword()) ::
          {:ok, Nx.Tensor.t()} | {:error, String.t()}
  def unet_forward(latents, timestep, prompt_embeds, pooled_embeds, opts \\ [])

  def unet_forward(latents, timestep, prompt_embeds, pooled_embeds, _opts)
      when is_number(timestep) do
    case validate_unet_options(latents, prompt_embeds, pooled_embeds) do
      :ok ->
        {:error, "Models not initialized. Call initialize_model() first."}
  ...
  def unet_forward(_latents, _timestep, _prompt_embeds, _pooled_embeds, _opts) do
  ...
  ## Returns
    ...
    * `{:ok, latents}` - Random latent tensor [1, 4, h//8, w//8]
  ...
  defp get_python_module_path do
    Path.join([:code.priv_dir(:margarine), "python", "sdxl_pythonx.py"])
  end
  ...
  defp validate_init_options(model_type, model_id, device, torch_dtype) do
    case {model_type, model_id} do
      ...
      {type, _} when type not in [:sdxl_base, :sdxl_turbo] ->
        {:error, "model_type must be :sdxl_base or :sdxl_turbo, got: #{inspect(type)}"}
  ...
  defp validate_dtype(:ok, dtype) when dtype in ["float16", "float32", "bfloat16"], do: :ok

  defp validate_dtype(:ok, dtype),
    do: {:error, "torch_dtype must be float16, float32, or bfloat16, got: #{dtype}"}

  defp validate_dtype(error, _dtype), do: error

  defp validate_prompt(""), do: {:error, "Prompt cannot be empty"}
  defp validate_prompt(_prompt), do: :ok

  defp validate_encode_options(guidance_scale) when is_number(guidance_scale), do: :ok
  defp validate_encode_options(_), do: {:error, "guidance_scale must be a number"}

  defp validate_unet_options(latents, prompt_embeds, pooled_embeds) do
    case validate_tensor(latents, "latents") do
